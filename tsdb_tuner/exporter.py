...
class TunerMetricsCollector:
    ...
    def get_last_session(self, cur) -> int | None:
        cur.execute("""
    ...
    def get_session_experiments(self, cur, session_id: int) -> list[int]:
        cur.execute("""
            SELECT DISTINCT experiment_id
            FROM public.optimization_trials
            WHERE session_id = %s AND experiment_id IS NOT NULL
        ...
        try:
            ...
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    session_id = self.get_last_session(cur)
                    if not session_id:
                        return
                    run_scope = self._get_run_scope(cur, session_id)
                    session_exp = self.get_session_experiments(cur, session_id)
                    if not session_exp:
                        return

                    yield from self.experiment_metrics(cur, run_scope, session_id)
                    yield from self.session_metrics(cur, session_id)
                    yield from self.generation_metrics(cur, session_id)
                    yield from self.container_metrics(cur, session_exp)
                    yield from self.pg_metrics(cur, session_exp)
                    yield from self.surrogate_metrics(cur, session_id)
            finally:
                conn.close()
        except Exception as exc:
            print(f"[exporter] collect error: {exc}")

    def experiment_metrics(self, cur, exp_ids, session_id):
        cur.execute("""
    ...
    def session_metrics(self, cur, session_id):
        cur.execute("""
            SELECT
                os.id::text AS sid,
                os.algorithm,
    ...
    def generation_metrics(self, cur, session_id):
        cur.execute("""
    ...
    def container_metrics(self, cur, exp_ids):
        cur.execute("""
    ...
    def pg_metrics(self, cur, exp_ids):
        cur.execute("""
    ...
    def surrogate_metrics(self, cur, session_id):
        cur.execute("""
